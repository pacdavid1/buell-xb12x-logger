...
.leg{display:flex;align-items:center;gap:6px;font-family:var(--mn);font-size:7px;color:var(--dm);padding:0 4px}
...
<button class="m3 on" data-m="mod">MOD</button>
<button class="m3" data-m="delta">DELTA</button>
</div>
<div class="c3"><canvas id="c3" height="300"></canvas></div>
<div class="leg" id="leg"></div>
</div>
</div>
<script>
var mB=null,mM=null,cur='fuel_front',m3='mod';
...
var cvs,ctx,ang=-0.65,tilt=0.9,drag=false,lx=0;
...
function init3D(){
  ...
  function rs(){var p=cvs.parentElement;cvs.width=p.clientWidth;cvs.height=300;draw3D()}
  rs();window.addEventListener('resize',rs);
  cvs.addEventListener('mousedown',function(e){drag=true;lx=e.clientX});
  window.addEventListener('mousemove',function(e){if(drag){ang+=(e.clientX-lx)*0.005;lx=e.clientX;draw3D()}});
  window.addEventListener('mouseup',function(){drag=false});
  cvs.addEventListener('touchstart',function(e){drag=true;lx=e.touches[0].clientX},{passive:true});
  cvs.addEventListener('touchmove',function(e){e.preventDefault();if(drag){ang+=(e.touches[0].clientX-lx)*0.005;lx=e.touches[0].clientX;draw3D()}},{passive:false});
...
function draw3D(){
  ...
  var sX=Math.min((W-60)/(C-1),30);
  var sY=Math.min((H-50)/(R-1),22);
  var sZ=(H*0.32)/(zX-zN);
  var ox=W*0.5,oy=H*0.82;
  var ca=Math.cos(ang),sa=Math.sin(ang),ct=Math.cos(tilt),st2=Math.sin(tilt);

  function pr(j,i,z){
    var x=(j-(C-1)/2)*sx,y=((R-1)-i-(R-1)/2)*sy,z2=(z-zN)*sz;
    var rx=x*ca-z2*sa,rz=x*sa+z2*ca;
    var ry=y*ct-rz*st2,rz2=y*st2+rz*ct;
    return[ox+rx,oy-ry,rz2];
  }
  ...
  function dcol(v){
    var ma=Math.max(Math.abs(zN),Math.abs(zX));if(ma===0)ma=1;
    var t=v/ma,r,g,b;
  ...
  for(var f=0;f<faces.length;f++){
    ...
    ctx.closePath();
    ctx.fillStyle=m3==='delta'?dcol(fc.v):heat(fc.v);
    ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,0.08)';ctx.lineWidth=0.5;ctx.stroke();
  }

  ctx.font='7px monospace';ctx.fillStyle='#666';
  for(var j=0;j<C;j+=2){var p=pr(j,R-1,zN);ctx.fillText(rpm[j],p[0]-8,p[1]+10)}
  for(var i=0;i<R;i+=2){var p=pr(0,i,zN);ctx.fillText(lod[i],p[0]-28,p[1]+3)}

  var leg=document.getElementById('leg');
  if(m3==='delta'){
    leg.innerHTML='<span>Empobrecido</span><div class="bar" style="background:linear-gradient(to right,rgba(30,60,255,.9),rgba(80,80,80,.5),rgba(255,60,60,.9))"></div><span>Enriquecido</span><span style="margin-left:8px">'+zN.toFixed(1)+' / +'+zX.toFixed(1)+'</span>';
  }else{
    leg.innerHTML='<span>'+zN.toFixed(0)+'</span><div class="bar" style="background:linear-gradient(to right,rgba(15,10,80,.9),rgba(15,70,220,.9),rgba(35,200,255,.9),rgba(155,255,100,.9),rgba(255,195,30,.9),rgba(255,50,20,.9))"></div><span>'+zX.toFixed(0)+'</span><span style="margin-left:8px;color:var(--a2)">'+(m3==='base'?'BASE':'MOD')+'</span>';
  }
}
...
print('tuner.html v4 escrito - mapa real con heatmap 3D')
