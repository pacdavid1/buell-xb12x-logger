#!/usr/bin/env python3
"""
CW2015 Driver - Battery Fuel Gauge (UPS-Lite v1.3)
I2C address: 0x62
Registers:
  0x00: VER - Version
  0x02: VCELL - Voltage (14-bit, 0.305mV/LSB)
  0x04: SOC - State of Charge (8.8 fixed point)
  0x08: STATUS - Bit4=CHG_IND (charging), Bit3=QUICKSTART
  0x0A: CONFIG/MODE - Bit7=SLEEP (1=sleeping, must clear on init)
"""
import time

class CW2015:
    I2C_ADDR  = 0x62
    REG_VER    = 0x00
    REG_VCELL  = 0x02
    REG_SOC    = 0x04
    REG_STATUS = 0x08
    REG_CONFIG = 0x0A

    def __init__(self, i2c_dev, i2c_addr=None):
        self._bus  = i2c_dev
        self._addr = i2c_addr or self.I2C_ADDR
        self._wake()

    def _wake(self):
        """Clear SLEEP bit (bit7 of CONFIG) if chip is sleeping."""
        try:
            cfg = self._bus.read_i2c_block_data(self._addr, self.REG_CONFIG, 1)[0]
            if cfg & 0x80:
                self._bus.write_byte_data(self._addr, self.REG_CONFIG, cfg & ~0x80)
                time.sleep(0.2)
        except Exception:
            pass

    def _read_word(self, reg):
        data = self._bus.read_i2c_block_data(self._addr, reg, 2)
        return (data[0] << 8) | data[1]

    def _read_byte(self, reg):
        return self._bus.read_i2c_block_data(self._addr, reg, 1)[0]

    def get_voltage(self):
        raw = self._read_word(self.REG_VCELL) & 0x3FFF
        return raw * 0.305 / 1000.0

    def get_soc(self):
        data = self._bus.read_i2c_block_data(self._addr, self.REG_SOC, 2)
        return data[0] + data[1] / 256.0

    def get_charging(self):
        """Bit4 of STATUS — driven by charger IC pin, not battery."""
        return bool(self._read_byte(self.REG_STATUS) & 0x10)

    def get_version(self):
        return self._read_word(self.REG_VER)

    def read_all(self):
        try:
            # Re-wake if chip fell asleep again (can happen after power glitch)
            cfg = self._read_byte(self.REG_CONFIG)
            if cfg & 0x80:
                self._bus.write_byte_data(self._addr, self.REG_CONFIG, cfg & ~0x80)
                time.sleep(0.1)

            charging = self.get_charging()
            v = round(self.get_voltage(), 3)
            s = round(self.get_soc(), 1)
            return {
                'bat_voltage':  v if 0 < v < 6 else None,
                'bat_soc':      s if 0 < s <= 100 else None,
                'bat_charging': charging,
            }
        except Exception:
            return {'bat_voltage': None, 'bat_soc': None, 'bat_charging': False}
