...
.w{flex:1;padding:8px;display:flex;flex-direction:column;gap:6px;overflow:hidden}
.cd{background:var(--p);border:1px solid var(--bd);padding:8px;border-radius:4px}
...
.ps{overflow-x:auto;overflow-y:auto;background:var(--p);border:1px solid var(--bd);border-radius:4px;flex-shrink:0;max-height:55vh}
...
.g3c{flex:1;min-width:320px;display:flex;flex-direction:column;border:1px solid var(--bd);border-radius:4px;overflow:hidden;background:var(--p)}
.g3c:first-child{border-right:none}
.g3c:last-child{border-left:none}
.g3t{font-family:var(--mn);font-size:8px;font-weight:700;color:var(--a2);padding:3px 6px;letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid var(--bd);background:rgba(255,255,255,.02);flex-shrink:0}
...
function render(){
  ...
  function tbl(data,cap){
    ...
    for(var i=0;i<rows;i++){h+='<tr><th class="r">'+lod[i]+'</th>';for(var j=0;j<cols;j++){var t=(data[i][j]-zN)/(zX-zN);h+='<td style="background:'+heatRGB(t)+'">'+data[i][j].toFixed(0)+'</td>'}h+='</tr>'}
    return h+'</table>';
  }
  function dtbl(){
...
function init3D(){
  ['c3b','c3d','c3m'].forEach(function(cid){
    ...
    function rs(){el.width=el.parentElement.clientWidth||300;el.height=el.parentElement.clientHeight-28||250;drawAll()}
  ...
  window.addEventListener('resize',function(){
    ['c3b','c3d','c3m'].forEach(function(cid){
      ...
      el.height=el.parentElement.clientHeight-28||250;
    });
    drawAll();
  });
}
...
function drawSurf(cid,data,rpm,lod,mode,legId){
  ...
  if(zX===zN)zX=zN+1;
  var pad=40,sX=Math.min((W-pad)/(Cl-1),Math.min(30,(W-pad)/(Cl-1))),sY=Math.min((H-pad)/(R-1),Math.min(24,(H-pad)/(R-1))),sZ=(H*0.35)/(zX-zN);
...
print('tuner.html v6 - heatmap en tablas + 3D separados grandes')
